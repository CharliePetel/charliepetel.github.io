...
const data = JSON.parse(fs.readFileSync("ausstellung.json", "utf8"));

let galleryHtml = "";

data.forEach((exhibition) => {
  const paragraphsHtml = exhibition.paragraphs.map((p) => `<p>${p}</p>`).join("\n");

  const imagesHtml = exhibition.images
  .map(img => `<img src="${img}" alt="${exhibition.title}">`)
  .join("\n");

const html = template
  .replace("{{title}}", exhibition.title)
  ...
  // Baue die Kachel für exhibitions.html
  galleryHtml += `
  <a class="tile" href="${exhibition.filename}">
    <img src="${exhibition.thumbnail}" alt="${exhibition.title}" />
    <div class="tile-title">${exhibition.title.toUpperCase()} – ${exhibition.subtitle}</div>
  </a>\n`;
});

// HTML-Rahmen für die exhibitions-Seite
...
<head>
  ...
  <title>Exhibitions – Charlie Petel</title>
